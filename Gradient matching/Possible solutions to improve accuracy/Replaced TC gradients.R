# Replaced TC gradients.R
# Author: Yunchen Xiao

# This .R file generates the parameter estimates under the circumstances
# of all tumour cells related gradients replaced by the corresponding true
# gradients calculated by the finite difference scheme.

# Environment settings
library(readr)
in.dir <- "SimRes_ests_converge_check"
new.dir <- "SimRes_replaceall_tc_ests_fds_grads"
n.sim <- 200
  ...
  for (j in 1:n.sim) {
    ...
    if(save.sims)
      readr::write_rds(list(grads = temp.grad.cv), 
                       path = paste0("./", new.dir, "/cv", i, "_sim", j, "_res_replace_n.rds"))
  }
}
...
for (i in 1:n.sim) {
  temp.grad.cv1 <- readr::read_rds(paste0(new.dir, "/cv", 1, "_sim", i, "_res_replace_n.rds"))$grad
...
for (i in 1:n.sim) {
  temp.grad.cv2 <- readr::read_rds(paste0(new.dir, "/cv", 2, "_sim", i, "_res_replace_n.rds"))$grad
...
for (i in 1:n.sim) {
  temp.grad.cv3 <- readr::read_rds(paste0(new.dir, "/cv", 3, "_sim", i, "_res_replace_n.rds"))$grad
...
for (i in 1:n.sim) {
  temp.grad.cv4 <- readr::read_rds(paste0(new.dir, "/cv", 4, "_sim", i, "_res_replace_n.rds"))$grad
...
for (i in 1:n.sim) {
  temp.grad.cv5 <- readr::read_rds(paste0(new.dir, "/cv", 5, "_sim", i, "_res_replace_n.rds"))$grad
...
write.table(cv.all.mean.replace, "Parameters means tc gradients replaced by FDS grads.txt")

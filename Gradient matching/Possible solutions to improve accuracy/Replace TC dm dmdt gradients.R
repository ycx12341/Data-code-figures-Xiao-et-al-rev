# Replaced TC dm dmdt gradients.R
# Author: Yunchen Xiao

# This .R file generates the parameter estimates under the circumstances of 
# all tumour cells related gradients, temporal gradients of MDE and the second
# order spatial gradient related to dm being replaced by the true gradients 
# calculated by the finite difference scheme.

# Environment settings
library(readr)
in.dir <- "SimRes_ests_converge_check"
new.dir <- "SimRes_replace_tc_and_dm_dmdt_fds_grads"
n.sim <- 200
...
for (i in 1:5) {
  for (j in 1:n.sim) {
    ...
    temp.grad.cv$grad.lhs_m <- as.matrix(ref.grad[, 469:546])
    temp.grad.cv$grad.rhs_dm <- as.matrix(ref.grad[, 547:624])
    
    if(save.sims)
      readr::write_rds(list(grads = temp.grad.cv), 
                       path = paste0("./", new.dir, "/cv", i, "_sim", j, "_res_replace_tc_and_dm_dmdt.rds"))
  }
}

start.values <- c(0.01, 0.133, 6.25, 12.5, 0.0166, 0.125)

# A function that calculates the sum of squared differences between
# the gradients
...
for (i in 1:n.sim) {
  temp.grad.cv1 <- readr::read_rds(paste0(new.dir, "/cv", 1, "_sim", i, "_res_replace_tc_and_dm_dmdt.rds"))$grad
...
for (i in 1:n.sim) {
  temp.grad.cv2 <- readr::read_rds(paste0(new.dir, "/cv", 2, "_sim", i, "_res_replace_tc_and_dm_dmdt.rds"))$grad
...
for (i in 1:n.sim) {
  temp.grad.cv3 <- readr::read_rds(paste0(new.dir, "/cv", 3, "_sim", i, "_res_replace_tc_and_dm_dmdt.rds"))$grad
...
for (i in 1:n.sim) {
  temp.grad.cv4 <- readr::read_rds(paste0(new.dir, "/cv", 4, "_sim", i, "_res_replace_tc_and_dm_dmdt.rds"))$grad
...
# Parameter estimates at the fifth level of cv, 0.1
res.cv5 <- vector()
fail.conv.cv5 <- vector()

for (i in 1:n.sim) {
  temp.grad.cv5 <- readr::read_rds(paste0(new.dir, "/cv", 5, "_sim", i, "_res_replace_tc_and_dm_dmdt.rds"))$grad
...
write.table(cv.all.mean.replace, "Parameters means tc dm dmdt gradients replaced by FDS grads.txt")
